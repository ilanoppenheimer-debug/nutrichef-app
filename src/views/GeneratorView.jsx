...
export default function GeneratorView() {
  ...
  const getSuggestions = async () => {
    ...
    const prompt = `Eres un chef IA. Genera 3 opciones de ${dishType} usando: ${ingredients}.
Cocina: ${cuisine}. Perfil: Obj:${profile.goals}${profile.dailyCalories ? ` Cal:${profile.dailyCalories}kcal` : ''}${profile.dietaryStyle !== 'Ninguna' ? ` Dieta:${profile.dietaryStyle}` : ''}${profile.allergies.length ? ` Alergias:${profile.allergies.join(',')}` : ''}${profile.dislikes.length ? ` Evitar:${profile.dislikes.join(',')}` : ''}${profile.budgetFriendly ? ' Economico:Si' : ''}.
...
{"suggestions":[{"id":1,"name":"...","type":"...","description":"..."}]}`;
  ...
  const generateFromSuggestion = async (sugg) => {
    ...
    const prompt = `Receta para "${sugg.name}". ${sugg.description}. Ingredientes base: ${ingredients}.
Perfil: Obj:${profile.goals}${profile.dailyCalories ? ` Cal:${profile.dailyCalories}kcal` : ''}${profile.proteinTarget ? ` Prot:${profile.proteinTarget}g` : ''}${profile.dietaryStyle !== 'Ninguna' ? ` Dieta:${profile.dietaryStyle}` : ''}${profile.allergies.length ? ` Alergias:${profile.allergies.join(',')}` : ''}${profile.dislikes.length ? ` Evitar:${profile.dislikes.join(',')}` : ''}${profile.useProteinPowder ? ' ProtPolvo:Si' : ''}.
${complexity.promptInstructions}
Devuelve SOLO este JSON:
